...
export default function ProfileScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Could not load your profile', readableError(error, 'Try again.'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    ...
    >
      <ScreenHeader title="Profile" subtitle="Identity, credits, and activity" right={<Button label="Sign out" icon={LogOut} variant="secondary" onPress={logout} />} />
      ...
      <View style={styles.infoGrid}>
        ...
        <Card style={styles.metric}>
          ...
          <Text style={styles.metricLabel}>I offer</Text>
        </Card>
        <Card style={styles.metric}>
          <ShieldCheck size={20} color={colors.accent} />
          <Text style={styles.metricValue}>{wantCount}</Text>
          <Text style={styles.metricLabel}>I want</Text>
        </Card>
      </View>
      ...
      <View style={styles.historyList}>
        {history.length === 0 ? (
          <Card>
            <Text style={styles.emptyText}>No credit movements yet.</Text>
          </Card>
